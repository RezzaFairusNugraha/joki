<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Mata Pelajaran - Reyka</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark mb-4">
        <div class="container">
            <a class="navbar-brand" href="/">🏠 SEKOLAH REYKA</a>
            <div class="navbar-nav">
                <a class="nav-link" href="/data_kelas">Data Kelas</a>
                <a class="nav-link" href="/data_murid">Data Murid</a>
                <a class="nav-link" href="/data_pengajar">Data Pengajar</a>
                <a class="nav-link" href="/mata_ajar">Mata Ajar</a>
                <a class="nav-link" href="/pengajar_mengajar">Penugasan</a>
                <a class="nav-link" href="/rekap_nilai">Rekap Nilai</a>
                <a class="nav-link" href="/info_sekolah">Info Sekolah</a>
            </div>
        </div>
    </nav>

    <div class="container mt-4">
        <h3>Daftar Mata Pelajaran</h3>
        <a href="/" class="btn btn-secondary btn-sm mb-3">Kembali</a>
        <a href="/mata_ajar/create" class="btn btn-primary btn-sm mb-3">Tambah Mapel</a>

        <div class="card p-3 shadow-sm border-0 border-top border-5 border-primary">
            <table class="table table-hover">
                <thead class="table-dark">
                    <tr>
                        <th>Mapel</th>
                        <th>Kategori</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
                @foreach($data as $m)
                <tr>
                    <td>{{ $m->nama_pelajaran }}</td>
                    <td>{{ $m->jenis_mapel }}</td>
                    <td>
                        <a href="/mata_ajar/{{ $m->mapel_id }}/edit" class="btn btn-sm btn-link">Edit</a>
                        <form action="/mata_ajar/{{ $m->mapel_id }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin hapus?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-link text-danger border-0 bg-transparent">Hapus</button>
                        </form>
                    </td>
                </tr>
                @endforeach
            </table>
        </div>
    </div>
</body>
</html>
