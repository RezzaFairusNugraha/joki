...
<html>
<head>
    <title>SIAKAD - Nilai Siswa</title>
    ...
    <div class="wrapper">
        ...
        <div class="content">
            <h2 class="d-flex justify-content-between">
                Data Nilai Siswa
                <a href="/nilai_siswa/create" class="btn btn-primary btn-sm">Input Nilai</a>
            </h2>
            <hr>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $n->murid ? $n->murid->nama : '-' }}</td>
                        <td>{{ $n->pelajaran ? $n->pelajaran->nama_pelajaran : '-' }}</td>
                        <td>{{ $n->nilai_uts }}</td>
                        <td>{{ $n->nilai_uas }}</td>
                        <td>{{ $n->nilai_akhir }}</td>
                        <td>
                            <a href="/nilai_siswa/{{ $n->id_nilai_siswa }}/edit" class="btn btn-sm btn-warning">Edit</a>
                            <form action="/nilai_siswa/{{ $n->id_nilai_siswa }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin hapus?')">
                                @csrf
                                @method('DELETE')
