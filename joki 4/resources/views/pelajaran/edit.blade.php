...
<body>
    <div class="wrapper">
        ...
        <div class="content">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="/pelajaran/{{ $mapel->id_pelajaran }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label>Nama Mata Pelajaran</label>
                            <input type="text" name="nama_pelajaran" value="{{ $mapel->nama_pelajaran }}" class="form-control" required>
                        </div>
                        <div class="mb-3">
                            <label>Tipe</label>
                            <select name="tipe" class="form-control" required>
                                <option value="Umum" {{ $mapel->tipe == 'Umum' ? 'selected' : '' }}>Umum</option>
                                <option value="Produktif" {{ $mapel->tipe == 'Produktif' ? 'selected' : '' }}>Produktif</option>
                            </select>
                        </div>
