    ...
    <div class="wrapper">
        ...
        <div class="content">
            <h2 class="d-flex justify-content-between">
                Data Berita
                <a href="/pengumuman/create" class="btn btn-primary btn-sm">Tambah Berita</a>
            </h2>
            <hr>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $b->judul }}</td>
                        <td>{{ $b->pengajar ? $b->pengajar->nama : '-' }}</td>
                        <td>{{ $b->tanggal }}</td>
                        <td>
                             <a href="/pengumuman/{{ $b->id_pengumuman }}/edit" class="btn btn-sm btn-warning">Edit</a>
                             <form action="/pengumuman/{{ $b->id_pengumuman }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin hapus?')">
                                 @csrf
                                 @method('DELETE')
